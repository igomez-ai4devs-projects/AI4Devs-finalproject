...
| `sla` | 1 | SLA policy resolution, timer state, pause/resume, warnings, breaches, recalculation on Priority change | `SlaPolicy`, `SlaInstance`, `SupportSchedule`; no UI of its own — surfaced inside ticket views |
...
| `libs/shared/domain` | Shared kernel primitives used by three or more contexts: `Identity`, `TicketReference`, `ImpactLevel`, `UrgencyLevel`, `Priority`, `DomainEvent`, `StateModel` (target — not built yet, `T-C10-10`), `ClockPort` | Pure TypeScript |
...
> **Status:** as in the readme, these components describe the **target architecture**, now partly scaffolded. What exists and is verified today:
>
> - **API** (`apps/api`, NestJS 11) and **Web Client** (`apps/web`, Angular 20 shell) — both lint and build green. The Web Client holds none of the responsibilities above and imports no library. The API holds only composition-root plumbing: the TypeORM data source (`apps/api/src/data-source.ts`, `synchronize: false`), the migration chain in `apps/api/src/migrations/` with a single bootstrap migration (the `iam` schema and the `citext` and `pg_trgm` extensions), and the in-process post-commit event dispatcher (`apps/api/src/event-dispatch/`), bound to `EventPublisherPort` through the `EVENT_PUBLISHER` token of `libs/shared/domain`; `apps/api/src/testing/` adds a dispatcher harness that is part of the module graph only when `NODE_ENV=test`. It exposes no product endpoint and no health probe. `api` runs 2 test suites / 15 tests.
> - **Shared libraries**, except `libs/shared/ui`, which does not exist yet: `libs/shared/util` (3 suites / 19 tests), `libs/shared/domain` (8 / 86 — every primitive listed in §4 except `StateModel`, plus `DateTimeRange`, `DomainError` and `EventPublisherPort`) and `libs/shared/contracts` (1 / 2 — error codes, error envelope, pagination, correlation id).
> - **The `incident` context** — its six libraries are scaffolded with their three tags and an empty barrel; they hold no code and nothing imports them.
> - **PostgreSQL 18** — provisioned by `docker/docker-compose.dev.yml` for development (host port 5452) and, for the API acceptance suite, as an ephemeral instance brought up, migrated and torn down by `pnpm nx e2e api-e2e`. Only the bootstrap migration exists; no context schema, table or TypeORM entity does.
>
> The dependency graph has 13 nodes and exactly two edges: `api → shared-domain` and `shared-domain → shared-util`. Still absent: every other bounded-context library, every domain model, use case and adapter, and every external gateway.
